...
app.controller('ProfileCtrl', [
  ...
  function ($scope, $rootScope, $modal, $http) {
    ...
    $rootScope.user = {};
    $http.get('/api/profile').success(function (user, status, headers, config) {
      console.log(user);
      $rootScope.user = user;
...
app.controller('LoginCtrl', [
  ...
  function ($scope, $rootScope, $http, $location, $window) {
    ...
    $scope.submit = function () {
      $http.post('/login', $scope.user).success(function (data, status, headers, config) {
        ...
        $scope.message = 'Nombre de usuario o Password inv\xe1lido';
        $scope.wellcome = '';
      });
    };
...
app.controller('RegisterCtrl', [
  ...
  function ($scope, $rootScope, $http, $window) {
    ...
    $scope.submit = function () {
      ...
      $http.post('/signup', user).success(function (data, status, headers, config) {
