...
/*    req.assert('direccion', 'required').notEmpty();
    ...
    */
    ...
    usr = new profile(req);
    db.get('users').update({ email: req.user.email },
                 { $set: { profile: usr }}
    );
    res.json({ res: 'OP_OK'});
  }
...
exports.find = function(db, name, email){
  return function(req, res){
  db.get('users').findOne({ name: req.params.name, email: req.params.email })
...
function findLogin(db, req, callback){
  ...
  db.get('users').findOne({ active: 1, email: req.body.email, password: md5(req.body.password) }, { id: 1, email: 1})
    .on('success', function(usr){
      if (usr){
        callback(usr);
      }
